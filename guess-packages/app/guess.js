...
//const GuessDatabase = require('wf-johnw-guess-db').GuessDatabase;
const Game = require('wf-johnw-guess-game').Game;
const GuessDatabase = require('wf-johnw-guess-db').GuessDatabase;
//console.log("DB Object:", db );

console.log("Imported GuessDatabase:", require('wf-johnw-guess-db'));
const db = new GuessDatabase(process.env.DBPATH);
console.log("DB Instance:", db); // Debugging
...
const start = (req, res) => {
    ...
    db.add_game(game);
    send_page(res, make_guess_page(game.id));
}
...
const guess = async (req, res) => {
    ...
    const record = db.get_game(req.body.gameId);
    ...
    db.add_guess(game, req.body.guess);
...
const history = (req, res) => {
    ...
    const records = db.get_games();

    for (const r of records) {
        r.guesses = db.get_game(r.id).guesses;
    }
...
const game_history = (req, res) => {
    ...
    */
    ...
    const record = db.get_game(req.query.gameId);
...
const r = db.get_games(); // Use the method defined in guess-db.js
console.log("Retrieved games:", r);
